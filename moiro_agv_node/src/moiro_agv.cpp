...
#include "moiro_agv_node/moiro_agv.hpp"

#include <memory>
#include <string>

using robotis::moiro_agv::MOIRO_AGV;
using namespace std::chrono_literals;

MOIRO_AGV::MOIRO_AGV(const std::string & usb_port)
: Node("moiro_agv_node", rclcpp::NodeOptions().use_intra_process_comms(true))
{
  RCLCPP_INFO(get_logger(), "Init MOIRO_AGV Node Main");
...
MOIRO_AGV::Wheels * MOIRO_AGV::get_wheels()
{
  return &wheels_;
}

MOIRO_AGV::Motors * MOIRO_AGV::get_motors()
{
  return &motors_;
}

void MOIRO_AGV::init_dynamixel_sdk_wrapper(const std::string & usb_port)
{
...
void MOIRO_AGV::check_device_status()
{
...
void MOIRO_AGV::publish_timer(const std::chrono::milliseconds timeout)
{
...
void MOIRO_AGV::heartbeat_timer(const std::chrono::milliseconds timeout)
{
...
void MOIRO_AGV::parameter_event_callback()
{
...
void MOIRO_AGV::cmd_vel_callback()
{
